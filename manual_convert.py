...
def extract_content_from_vs_read_file(read_file_content):
    """
    Extract the actual content from the read_file tool output
    
    Args:
        read_file_content: Content returned by the VS Code read_file tool
    
    Returns:
        Extracted content without VS Code tool wrappers
    """
    # Remove the read_file header and footer
    lines = read_file_content.split('\n')
    
    # Skip the first line if it's a comment with the filepath
    start = 1 if lines[0].startswith('<!-- filepath:') else 0
    
    # Return the actual content
    return '\n'.join(lines[start:])

# Main notebook contents - extracted from VS Code read_file tool
notebook_02_individual = """
<VSCode.Cell id="19c58b10" language="python">
from nbdev import *
default_exp individual
</VSCode.Cell>
<VSCode.Cell id="3466a8f1" language="python">
#hide
import sys
sys.path.append("..")
import json
...
<VSCode.Cell id="5de69051" language="markdown">
# DHPCTIndividual

> Class for implementing Deep Perceptual Control Theory individuals.
</VSCode.Cell>
<VSCode.Cell id="9454295a" language="python">
#export
...
# Convert and save the individual notebook
output_file = os.path.join(output_dir, "jupyter_02_individual.ipynb")
print(f"Absolute path of output file: {os.path.abspath(output_file)}")
convert_vscode_to_jupyter(notebook_02_individual, output_file)

# Now let's try to load it to verify
try:
    print("\nVerifying the converted notebook...")
    with open(output_file, 'r', encoding='utf-8') as f:
        loaded_nb = json.load(f)
    print(f"Successfully loaded the notebook - it has {len(loaded_nb['cells'])} cells")
    
    # Try to run nbdev_export on this specific file
    try:
        import subprocess
        print("\nAttempting to run nbdev_export on the converted notebook...")
        cmd = ["nbdev_export", "--path", output_file]
    ...
    print(f"Error verifying notebook: {str(e)}")
